...
- [ ] **[BUG - PRIORITY] Left-edge triangles not updating color** — confirmed still broken. Symptom: triangles on the far left flash the new color during the fold animation, but then revert to the original color when the fold completes. The rest of the screen updates correctly. Previous fix attempt (redirecting `anim.newColor` for mid-fold triangles) did not solve it. Root cause likely in the offscreen static cache: when a triangle completes its fold, `patchStaticTriangle()` or the cache rebuild is probably using stale color data or wrong canvas coordinates for left-edge triangles (which have negative x offsets due to the bleed zone). Need to add debug logging to trace exactly what color is being committed vs what's being painted to the static cache for left-edge triangle indices.
